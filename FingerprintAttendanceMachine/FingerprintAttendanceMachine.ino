...
char dst;//Initialize boolean variables
...
void setup(){
	...
	Serial.println(finger.baud_rate),//Print fingerprint sensor parameters
	...
	while(!sd.exists("LAST"))
		sd.mkdir("LAST");
	lcd.begin()
	,lcd.backlight();//Start the LCD screen
	rtc.begin(),//Start the clock module
	pinMode(BUZZER,OUTPUT),//Initialize the buzzer pin
	pinMode(SWITCH,INPUT_PULLUP),//Initialize the enroll switch
	lcd.setCursor(5,0),
	lcd.print("Ready!"),//Notify
...
void loop(){
	if(Serial.available()){//Read the commands
		lcd.clear(),
		lcd.setCursor(6,0),
		lcd.print("Menu");//Show 'Menu' on the LCD screen
		...
		if(command!=""){
			if(command=="help")//Show help
				...
				while(!data);//Stop if failed to open the file
				dst=!dst,
				data.write(dst);//Change the bit of the DST variable
			}else if(command=="saved fingerprints")//Show the number of saved fingerprints
				Serial.print("Saved fingerprints count: "),
				...
				while(data.available()){//Repeat this while there are bytes available
					Serial.print("User #"),
					Serial.print(data.position()+1),
					Serial.print(": ");//Print the number of the user
				...
				while(!Serial.available());
				uint16_t clock[6];
				clock[0]=Serial.parseInt(),
				clock[1]=Serial.parseInt(),
				clock[2]=Serial.parseInt(),
				clock[3]=Serial.parseInt(),
				clock[4]=Serial.parseInt(),
				clock[5]=Serial.parseInt(),
				rtc.adjust(DateTime(clock[0],clock[1],clock[2],clock[3],clock[4],clock[5])),
				Serial.println("Done!");
				...
				if(sd.exists("SUM/"+String(ID))){
					...
					data.close(),
					Serial.print("Days: "),
					Serial.println(time/86400),
					Serial.print("Hours: "),
					Serial.println(time%86400/3600),
					Serial.print("Minutes: "),
					Serial.println(time%3600/60),
	...
	if(!digitalRead(SWITCH))//Enroll a new fingerprint if the enroll switch is enabled
	...
	if(getFingerprintID()){//Read fingerprints
		...
		lcd.setCursor(0,1),
		lcd.print("User #"),
		lcd.print(finger.fingerID);//Show the user number on the LCD screen
		...
		digitalWrite(BUZZER,HIGH),//Make a beeping sound using the buzzer
		delay(1000),//Wait 1 second
		digitalWrite(BUZZER,LOW);//Stop the beeping
	}
	...
	lcd.print(now.month()),
	lcd.print('/'),
	lcd.print(now.day());//Show the date on the LCD screen
	...
	lcd.print(now.second()%10);//Show the time on the LCD screen
	return;
}
...
char getFingerprintID(){//Initialize the fingerprint reading function
	...
	if(p!=FINGERPRINT_OK){//Return if there's no match for the detected fingerprint
		lcd.clear(),
		lcd.print("No match!"),//Alert on the LCD screen
		delay(1000);//Wait 1 second
...
void getFingerprintEnroll(const unsigned id,const char Switch){//Initialize the fingerprint enroll function
	...
	while(p!=FINGERPRINT_OK){//Read until a fingerprint has been read
		if(Switch&&digitalRead(SWITCH))//Return if the switch is disabled
			return;
	...
	while(p!=FINGERPRINT_OK){//Read until a fingerprint is detected
		if(digitalRead(SWITCH))//Return if the switch is disabled
			return;
		p=finger.getImage();//Read a fingerprint
		switch(p){
			case FINGERPRINT_OK://(A fingerprint is detected)
